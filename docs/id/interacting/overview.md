...
Monero project nicely decouples network node logic from wallet logic. Wallet
logic is offered through three independent user interfaces - the GUI, the CLI,
and the HTTP API.
...
There are quite a few ways you can interact with Monero software. Perhaps the
most surprising for newcomers is that `monerod` daemon accepts interactive
keyboard commands while it is running.

Also, please note that `monerod` and `monero-wallet-rpc` are both accessible via
their respective HTTP API / JSON-RPC endpoints.
...
* the `bitmonero` directory name is a historical artifact from before Monero
  forked away from Bitmonero
...
It can also contain subdirectories for stagenet and testnet, mirroring the same
structure:
