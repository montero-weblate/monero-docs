...
trusted `blockchain.raw` file that you
[exported](../interacting/monero-blockchain-export-reference.md) earlier.

Please note that importing `blockchain.raw` will **not** speed up the process
over syncing up from p2p network. This is because usual bottlenecks are disk IO
and verification, not the download.
